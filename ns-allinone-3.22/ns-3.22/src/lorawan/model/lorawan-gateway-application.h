...
typedef struct LoRaWANEndDeviceInfoNS {
  LoRaWANEndDeviceInfoNS () : m_deviceAddress(), m_rx1DROffset(0), m_lastDSGW(NULL), m_lastGWs(),
...
public:
  ...
  static void clearLoRaWANNetworkServerPointer () { LoRaWANNetworkServer::m_ptr = NULL; }
...
private:
  ...
  //std::map < uint32_t, double > m_endDevices;
  //std::unordered_map < uint32_t, LoRaWANEndDeviceInfoNS > m_endDevices;
  std::map < uint32_t, LoRaWANEndDeviceInfoNS > m_endDevices;
  uint16_t m_pktSize;
