...
def build_scenario(n_bbu, n_bs, n_clusters, n_ue):
    ...
    add_macrocells(grid, DMACROMACRO, n_bs,  macrocells_center)
    add_clusters(grid, macrocells_center, n_clusters)
    add_users(grid, macrocells_center, n_bs, n_clusters, n_ue)

    return grid
"""
Add Macrocells
"""
def add_macrocells(grid, radius, n_bs, macrocells_center):
...
Add Clusters
"""
def add_clusters(grid, macrocells_center, n_clusters):
    count_clusters = 0
    p_clusters = list()
    reset = 0;
...
Add Users
"""
def add_users(grid, macrocells_center, n_bs, n_clusters, n_ue):
    count_ue = 0
    p_users = list()
    ...
    for i in range(0, n_bs):
        ...
        for j in range(0,len(p_users)):
            ...
            if (random.randint(0,100) > 50):
                ...
                tech = Sensor.NBIOT
            u = Sensor(j+1, p_users[j], None, tech, app, dist)
            grid.add_user(u)
    ...
    for ue in ues:
        for i in range (0, index):
            ...
            for sensor in grid._user:
                if (sensor.technology == Sensor.LORA):
                    env.process(transmit_lora(env, sensor))
                else:
                    env.process(transmit_lora(env, sensor))
